...
import styles from './StaffModalForm.module.scss'
...
interface IStaffModalForm {
	staffId: number
	handlerClose(): void
}

export function StaffModalForm({ staffId, handlerClose }: IStaffModalForm) {
	...
	const { mutate, isLoading } = useMutation(
		async (values: IStaffInitialValues) => {
			...
			const response = await StaffService.changeStaff(staffId, formData)
